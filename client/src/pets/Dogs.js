...
const Dogs = ({ navigation }) => {
  ...
  const fetchData = () => {
    ...
    axios.get('https://adoptails-server.onrender.com/getPets').then((response) => {
      // Filter for only Dogs (you may need to adjust the condition)
      const Dogs = response.data.filter((pet) => pet.petCategory === 'Dog');
      setPetData(Dogs);
      setLoading(false);
    });
  };
  ...
  const goToPetDetails = (item) => {
    // Pass the selected pet's data to the PetDetails screen
      ...
      <ScrollView style={styles.scrollView}>
        {petData.length === 0 ? (
          <View style={styles.noPetsContainer}>
            <Text style={styles.noPetsText}>No Dogs available</Text>
          </View>
        ) : (
          petData.map((item) => (
