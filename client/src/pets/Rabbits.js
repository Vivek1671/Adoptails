...
const Rabbits = ({ navigation }) => {
  ...
  const fetchData = () => {
    ...
    axios.get('https://adoptails-server.onrender.com/getPets').then((response) => {
      // Filter for only Rabbits (you may need to adjust the condition)
      const Rabbits = response.data.filter((pet) => pet.petCategory === 'Rabbit');
      setPetData(Rabbits);
      setLoading(false);
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        {petData.length === 0 ? ( // Check if there are no rabbits
          <View style={styles.noPetsContainer}>
            <Text style={styles.noPetsText}>No Rabbits available</Text>
          </View>
        ) : (
          petData.map((item) => (
