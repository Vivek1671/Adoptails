...
const Fishes = ({ navigation }) => {
  ...
  const fetchData = () => {
    ...
    axios.get('https://adoptails-server.onrender.com/getPets').then((response) => {
      // Filter for only Fishes (you may need to adjust the condition)
      const Fishes = response.data.filter((pet) => pet.petCategory === 'Fish');
      setPetData(Fishes);
      setLoading(false);
    });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView}>
        {petData.length === 0 ? ( // Check if there are no fishes
          <View style={styles.noPetsContainer}>
            <Text style={styles.noPetsText}>No Fishes available</Text>
          </View>
        ) : (
          petData.map((item) => (
