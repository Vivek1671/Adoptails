...
const Cats = ({ navigation }) => {
  ...
  const fetchData = () => {
    ...
    axios.get('https://adoptails-server.onrender.com/getPets').then((response) => {
      // Filter for only Cats (you may need to adjust the condition)
      const Cats = response.data.filter((pet) => pet.petCategory === 'Cat');
      setPetData(Cats);
      setLoading(false);
    });
  };
      ...
      <ScrollView style={styles.scrollView}>
        {petData.length === 0 ? (
          <View style={styles.noPetsContainer}>
            <Text style={styles.noPetsText}>No Cats available</Text>
          </View>
        ) : (
          petData.map((item) => (
